...
class ConcreteDecoratorJSON(Decorator):
    ...
    def __str__(self, currencies_ids_lst=None):  # JSON
        return f"ConcreteDecoratorJSON: {json.dumps(self.wrapped_object.get_currencies(currencies_ids_lst), indent=4, ensure_ascii=False)}"


class ConcreteDecoratorCSV(Decorator):
    """ConcreteDecoratorB"""

    def __str__(self, currencies_ids_lst):
        json_info = json.dumps(self.wrapped_object.get_currencies(currencies_ids_lst), indent=4, ensure_ascii=False)
        print('ConcreteDecoratorCSV: ')
        return pd.read_json(json_info).to_csv(sep='\t')
